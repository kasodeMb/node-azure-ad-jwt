...
function AzureActiveDirectoryValidationManager() {
    ...
    self.requestOpenIdConfig = async (tenantId, cb) => {
        ...
        try {
            const result = await fetch(tenantOpenIdconfig.url, { headers: { 'Content-Type': 'application/json' } });
            const data = await result.json();
            console.log('requestOpenIdConfig', data);
            cache.put(tenantOpenIdconfig, data);
            return data;
        } catch (error) {
            throw error;
        }
    ...
    self.requestSigningCertificates = async (jwtSigningKeysLocation, options) => {
        ...
        try {
            ...
            if (options && options.kid) {
                ...
                data.keys.forEach(function (publicKeys) {
